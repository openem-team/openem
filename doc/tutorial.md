...
```shell
nvidia-docker run --name openem --rm -ti -v <Path to working-dir>:/data cvisionai/openem_lite bash
```

If using any X11 code, it is important to also enable X11 connections within the docker image:
```shell
nvidia-docker run --name openem --rm -ti -v <Path to working-dir>:/data -v"$HOME/.Xauthority:/root/.Xauthority:rw" --env=DISPLAY --net=host cvisionai/openem_lite bash
```
...
At this point the split script will print out useful analytics to verify the feasibility of your training set. Some considerations for building a succesful training 
set: 
- If multi-class detection is used, it is important to be coignizant of population count of each class.
- If footage varies by camera, time, a blend across the scenarios is a good idea to be representatitive of operational conditions.
