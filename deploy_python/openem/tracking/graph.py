...
def _compute_weights(
    ...
    for i, (t0, t1) in bar(enumerate(pairs)):
        ...
        if num_dets == 1:
            ...
            d0_app = np.array([app_normalizer(tracklets[t0][k]) for k in range(-num_dets, 0)])
            d1_app = np.array([app_normalizer(tracklets[t1][k]) for k in range(0, num_dets)])
            d0_st = np.array([[
    ...
    num_batches = math.ceil(len(detection_w_idx)/batch_size)
    ...
    num_batches = math.ceil(len(detection_y_idx)/batch_size)
    bar=progressbar.ProgressBar(max_value=num_batches,prefix="batches")
    for bn in bar(range(num_batches)):
        start=bn*batch_size
        end=start+batch_size
        ...
        feed_dict={track_comparator.d0_app: d0_app,
                   track_comparator.d0_st: d0_st,
                   track_comparator.d1_app: d1_app,
                   track_comparator.d1_st: d1_st}
        res = track_comparator.session.run([track_comparator.output],feed_dict=feed_dict)[0][0]
