...
def process_media(
    ...
    for track in track_mgr.final_track_list:
        ...
        logger.info(f"Track confidence info:")
        logger.info(f"    Number of non-tracker detections - {num_detections}")
        logger.info(f"    Total sum confidence - {total_confidence}")
        if num_detections < args.min_num_detections or total_confidence < args.min_total_confidence:
            continue
